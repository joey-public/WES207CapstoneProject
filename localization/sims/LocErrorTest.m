...
ht = 20; %height of highest receiver (z dimension, meters)
height_diff = 10; %difference of height between each receiver (meters)
numTrials = 100000;
...
%     hold off
    ...
    %error in meters
    err1 = sqrt((real(loc_est1(1))-x)^2+(real(loc_est1(2))-y)^2);
    err2 = sqrt((real(loc_est2(1))-x)^2+(real(loc_est2(2))-y)^2);

    if err1 <= err2
        est1better = est1better+1;
    else
        %keyboard
    end

    totalerror = totalerror + err1;
    error_mags(iteration) = err1;
end

average_error = totalerror/numTrials
median_error = median(error_mags)
est1better_percent = est1better/numTrials