...
%%
% read data from file
datafile = fopen('usrp_pps_data_joey_20230515/usrp_pps_data_10m.dat'); % filename of USRP capture File
datafile2 = fopen('usrp_pps_data_vinit/usrp_pps_data10.dat'); % filename of USRP capture File
...
realdata = samples(1:2:end);
imagdata = samples(2:2:end);

clear samples

realdata2 = samples2(1:2:end);
imagdata2 = samples2(2:2:end);
...
startidx_rec2 = find(abs(data2(idxoffset+1:end))>threshold,1,'first');
...
%% Cross Correlation
[xc, idxs]= xcorr(filtered_data,filtered_data2);
figure(100)
plot(idxs, abs(xc))
[~, maxidx] = max(abs(xc));
grid on
...
[xc2, idxs2] = xcorr(abs(filtered_data),abs(filtered_data2));
figure(102)
plot(idxs2,xc2)
grid on
xlabel("Sample Offset")
